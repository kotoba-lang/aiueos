(ns aiueos.provider.cloud
  ...
  **A request that could not complete is not a response.** Timeouts and I/O
  ...
  It does not authenticate the endpoint beyond what the JDK's default trust
  store does — the machine's trust in the cloud is the platform's trust store,
  which aiueos neither owns nor pins. It does not write: `PUT /ipfs/:cid` needs
...
(def errors
  ...
  fault means there was nothing to decide about."
...
(defn client
  "An HTTP client that does not follow redirects. `NEVER` is the builder's
  default; it is set explicitly because a reader cannot see a default, and this
  one is load-bearing."
  ^HttpClient [opts]
          ...
          outcome (cloud/perform
                   ...
                   (fn [request]
                     (let [builder (-> (HttpRequest/newBuilder)
                                       ...
                                       (.GET))]
                       (send-request http (.build builder) opts))))]
      (if (:ok? outcome)
