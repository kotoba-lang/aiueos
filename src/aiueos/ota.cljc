(ns aiueos.ota
  "The end-to-end update: what `aiueos.publisher` admits, what `aiueos.update`
  ...
  `aiueos.update/admit-step` takes `:publisher-admitted?` as a bare boolean, and
  ...
  `aiueos.signing/sha256-hex` is JVM-only and a second canonicaliser would be a
  signature-confusion hazard (`aiueos.key-lifecycle/document-bytes` says so in
  ...
  (:require [aiueos.publisher :as publisher]
            [aiueos.update :as update]))

(def deny-reasons
  "Reasons `aiueos.ota` itself produces. Reasons from `aiueos.publisher` and
  `aiueos.update` pass through unchanged — a caller reading a verdict should not
...
(defn verify-artifacts
  ...
  **mismatch**, not a skip. A partially-fetched update that verified the half it
  ...
  applying nothing — the same rule `aiueos.update/unknown-kinds` states for
...
(defn admit
  "Ask `aiueos.publisher` about a release, and stamp the answer with the
...
(defn advance
  ...
  `ota` — `{:manifest-id :admission :step :class :kinds :observed-digests
            :artifacts :now-ms}` plus everything `aiueos.update/admit-step`
