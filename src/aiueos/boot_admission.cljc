(ns aiueos.boot-admission
  ...
  current."
  (:require [aiueos.ota :as ota]
            [aiueos.publisher :as publisher]))
...
(def deny-reasons
  "Reasons this namespace produces. `aiueos.publisher` reasons pass through
  unchanged."
...
(defn admit-boot
  ...
  `state`    — `aiueos.publisher`'s state (root, revocation bits, now-ms)
