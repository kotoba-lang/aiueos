...
void aiueos_kernel_main(const struct aiueos_boot_info *boot) {
  ...
  if (!boot || boot->magic != 0x414955454f53424fULL || boot->version != 1 ||
    ...
    debug_string("AIUEOS_PROCESS_REAP_OK tasks=2 slots=8 generations=reused owner-caps-revoked allocator-pages=10 zero-reused kernel-stacks=zero\n");
    serial_string("AIUEOS_PROCESS_REAP_OK tasks=2 slots=8 generations=reused owner-caps-revoked allocator-pages=10 zero-reused kernel-stacks=zero\r\n");
