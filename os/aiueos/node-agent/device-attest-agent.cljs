...
;; Where to write the final outcome as a keyword. Exit codes deliberately
;; collapse outcomes a supervisor should treat the same -- `proved` and `idle`
;; are both 0, because being told there is nothing to prove is a successful
;; run. A caller that needs to tell them apart needs the outcome, not the code,
;; and reading it out of human-readable output would be a parser waiting to
;; disagree with the thing it parses.
(def outcome-file (opt "--outcome-file"))
...
(defn- finish! [outcome]
  (when outcome-file
    (try (fs/writeFileSync outcome-file (name outcome)) (catch :default _ nil)))
  (js/process.exit (exit-for outcome)))
