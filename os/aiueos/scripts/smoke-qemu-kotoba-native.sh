...
if [ "$expected_marker" = MPRC ]; then
  echo "AIUEOS_KOTOBA_NATIVE_QEMU_OK no-c-boot-chain memory-map-v2 allocator-pages=6 ownership-bitmap page-table-root identity-1g guard-unmapped text-rx state-rw-nx nxe cr0-wp cr3-activated invlpg reuse double-free-rejected zero-before-publish exit-boot-services"
else
