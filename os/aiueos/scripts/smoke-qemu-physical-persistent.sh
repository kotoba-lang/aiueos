#!/bin/sh
set -eu

repo=$(CDPATH= cd -- "$(dirname -- "$0")/../../.." && pwd)
aiueos="$repo/os/aiueos"
qemu=${QEMU_SYSTEM_X86_64:-qemu-system-x86_64}
timeout_cmd=$(command -v timeout || command -v gtimeout || true)
[ -n "$timeout_cmd" ] || {
  echo "error: timeout or gtimeout is required" >&2
  exit 1
}
command -v "$qemu" >/dev/null 2>&1 || {
  echo "error: qemu-system-x86_64 is required" >&2
  exit 1
}

if [ -z "${OVMF_CODE:-}" ]; then
  for candidate in /opt/homebrew/share/qemu/edk2-x86_64-code.fd \
    /usr/share/OVMF/OVMF_CODE_4M.fd /usr/share/OVMF/OVMF_CODE.fd; do
    [ -f "$candidate" ] && { OVMF_CODE=$candidate; break; }
  done
fi
if [ -z "${OVMF_VARS:-}" ]; then
  for candidate in /opt/homebrew/share/qemu/edk2-i386-vars.fd \
    /usr/share/OVMF/OVMF_VARS_4M.fd /usr/share/OVMF/OVMF_VARS.fd; do
    [ -f "$candidate" ] && { OVMF_VARS=$candidate; break; }
  done
fi
[ -f "${OVMF_CODE:-}" ] || { echo "error: OVMF code not found" >&2; exit 1; }
[ -f "${OVMF_VARS:-}" ] || { echo "error: OVMF vars template not found" >&2; exit 1; }

work=$(mktemp -d "${TMPDIR:-/tmp}/aiueos-native-persistent-smoke.XXXXXX")
trap 'rm -rf "$work"' EXIT HUP INT TERM
mkdir -p "$work/esp/EFI/BOOT"

AIUEOS_OUT="$work/release" \
AIUEOS_PERSISTENT_BOOT=1 \
AIUEOS_ALLOW_DIRTY_QUALIFICATION_BUILD=${AIUEOS_ALLOW_DIRTY_QUALIFICATION_BUILD:-0} \
SOURCE_DATE_EPOCH=0 \
  "$aiueos/scripts/build-physical-qualification-pxe.sh" >/dev/null
cp "$work/release/aiueos-k16-native-pxe.efi" \
  "$work/esp/EFI/BOOT/BOOTX64.EFI"
cp "$OVMF_VARS" "$work/vars.fd"

set +e
# This gate hashes the whole embedded kernel with the deliberately small
# freestanding SHA-256 path before entry.  The native inference glyph/status
# object moved the TCG run past the old 18-second wall without changing its
# progress markers; 35 seconds is the measured deterministic envelope on the
# same Mac, not a relaxed success condition.
"$timeout_cmd" 35 "$qemu" -machine q35,accel=tcg -cpu max -m 256M -smp 2 \
  -drive "if=pflash,format=raw,readonly=on,file=$OVMF_CODE" \
  -drive "if=pflash,format=raw,file=$work/vars.fd" \
  -drive "format=raw,file=fat:rw:$work/esp" \
  -display none -serial "file:$work/native.serial" \
  -debugcon "file:$work/native.debug" -global isa-debugcon.iobase=0xe9 \
  -monitor none -no-reboot >/dev/null 2>&1
native_rc=$?
set -e
[ "$native_rc" -eq 124 ] || {
  echo "error: persistent native EFI did not remain running (rc=$native_rc)" >&2
  exit 1
}
for marker in \
  "AIUEOS_LOADER_WATCHDOG_DISABLED persistent-native" \
  "AIUEOS_NETBOOT_EMBEDDED_OK kernel+initramfs sha256-v1" \
  "AIUEOS_PHYSICAL_QUALIFICATION_OK native-core-v2 internal-disk-writes=none"; do
  grep -F "$marker" "$work/native.debug" >/dev/null || {
    echo "error: persistent native evidence is absent: $marker" >&2
    exit 1
  }
done
if grep -F "AIUEOS_LOADER_WATCHDOG_ARMED" "$work/native.debug" >/dev/null; then
  echo "error: persistent native EFI armed the recovery watchdog" >&2
  exit 1
fi
python3 - "$work/release/aiueos-k16-native-pxe-receipt.json" <<'PY'
import json
from pathlib import Path
import sys

receipt = json.loads(Path(sys.argv[1]).read_text(encoding="ascii"))
assert receipt["return"]["bootnext"] == "current-pxe-persistent"
assert receipt["return"]["watchdog"] == "disabled"
assert receipt["safety"]["internal-disk-writes"] is False
PY

printf '%s\n' \
  "AIUEOS_NATIVE_PERSISTENT_QEMU_OK watchdog=disabled reset=none internal-ssd-writes=none physical-k16=unverified"
