...
(defn- drive [sock]
  (let [rxbin (atom []) i-c (atom nil) i-s (atom nil) phase (atom :banner)
        sk (atom nil) sc-seq (atom 0) cs-seq (atom 0)]  ; s->c and c->s GCM counters
    ...
    (letfn [(unenc-packets! []             ; pull complete unencrypted packets
            ...
            (seal-cs [payload]              ; seal one c->s packet with the running counter
              (let [w (rec/seal gcm-encrypt (:key-c->s @sk) (:iv-c->s @sk) @cs-seq payload (repeat 0))]
                (swap! cs-seq inc) (->buf w)))
            (start-encrypted [k h]
              ...
              (js/setTimeout
               (fn [] (clog "sending encrypted SERVICE_REQUEST")
                 (.write sock (seal-cs (ua/service-request-payload ua/service-userauth))))
               650))
            (send-userauth [h]
              (let [point (kex/ec-point (->vec (hexbuf auth-x)) (->vec (hexbuf auth-y)))
                ...
                (.write sock (seal-cs req))))
            (open-channel []                ; after USERAUTH_SUCCESS: open a session channel
              (clog "logged in; opening session channel")
              (.write sock (seal-cs (con/channel-open-payload "session" 0 2097152 32768))))
            (send-exec []                   ; after CONFIRMATION: run the command
              (clog "channel confirmed; sending exec" (pr-str command))
              (.write sock (seal-cs (con/channel-request-exec-payload 0 true command))))
                  ...
                  (if (nil? p)
                    ...
                    (do (clog "decrypted s->c msg" (first p))
                        (cond
                          ...
                          (= ua/msg-userauth-success (first p)) (open-channel)
                          (= ua/msg-userauth-failure (first p)) (finish false "userauth-failure")
                          (= con/msg-channel-open-confirmation (first p)) (send-exec)
                          (= con/msg-channel-data (first p))
                          (let [d (con/parse-channel-data p)
                                out (apply str (map char (:data d)))]
                            (clog "CHANNEL_DATA:" (pr-str out))
                            (if (= out expect-output)
                              (finish true "channel-data-matches")
                              (finish false (str "channel-data mismatch: " (pr-str out)))))))))))]
      (let [h-atom (atom nil)]
        (.on sock "data"
             (fn [d]
               (cond
...
(when-not (str/includes? (.toString (.readFileSync fs kernel)) "AIUEOS_SSH_SESSION_OK")
  (die "kernel has no session marker; rebuild with AIUEOS_SSH_LISTEN=1"))
(js/setTimeout pool-tick 5000)
(println "AIUEOS_SSH_SESSION_GATE booting; client runs a command over" host-port)
(def env (doto (js/Object.assign #js {} js/process.env)
           ...
           ;; the session boot runs the kex/userauth crypto AND the channel during
           ;; enumeration; give it room to reach qemu_exit so the serial is complete.
           (aset "AIUEOS_QEMU_TIMEOUT" "1500")))
;; detached so the whole boot (smoke-qemu-uefi.sh -> timeout -> qemu) is a process
;; group we can kill on early conclude -- otherwise a killed wrapper leaves QEMU
;; holding the hostfwd port and the next run's client connects to the stale guest.
(def child (.spawn cp (.join path aiueos "scripts" "smoke-qemu-uefi.sh") #js []
                   #js {:stdio "inherit" :env env :detached true}))
(defn- kill-group [] (try (.kill js/process (- (.-pid child)) "SIGKILL") (catch :default _ nil)))
(def concluded (atom false))
(defn- conclude [code]
  (when (compare-and-set! concluded false true)
    (let [serial (if (.existsSync fs serial-log) (str/replace (.readFileSync fs serial-log "utf8") "\r" "") "")
      ...
      (when-not kernel-ok
        (println "AIUEOS_SSH_SESSION_GATE line="
                 (pr-str (->> (str/split-lines serial) (filter #(str/includes? % "AIUEOS_SSH")) last))))
      (kill-group)
      (if (and kernel-ok (= reason "channel-data-matches"))
        (do (println "AIUEOS_SSH_SESSION_SMOKE_OK an independent client ran a command (exec) over a session channel and got its output back from the kernel")
            (js/setTimeout (fn [] (.exit js/process 0)) 300))
        (do (println "AIUEOS_SSH_SESSION_SMOKE_FAIL kernel-ok=" kernel-ok "client-ok=" authed "reason=" reason)
            (js/setTimeout (fn [] (.exit js/process 1)) 300))))))
;; Conclude as soon as the proof is complete -- the client logged in and got the
;; command output, and the kernel printed the marker -- rather than waiting for
;; the (very slow, crypto-during-enumeration) boot to reach qemu_exit, which past
;; the timeout triggers a reboot that overwrites the serial.
(def watch (js/setInterval
            (fn [] (when (and (:done @result) (= "channel-data-matches" (:reason @result)))
                     (let [serial (if (.existsSync fs serial-log) (.readFileSync fs serial-log "utf8") "")]
                       (when (str/includes? serial "AIUEOS_SSH_SESSION_OK") (conclude 0)))))
            1000))
(.on child "exit" (fn [code _] (conclude code)))
