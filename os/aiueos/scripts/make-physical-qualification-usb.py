...
def verify(path, probe_path, loader_path, kernel_path, initramfs_path):
    ...
    print("AIUEOS_PHYSICAL_QUALIFICATION_USB_OK probe-native-result-v8 "
...
def build(args):
    ...
    receipt = {
        "schema": "aiueos.physical-qualification-usb-receipt.v8",
        "created": datetime.fromtimestamp(epoch, timezone.utc).isoformat().replace("+00:00", "Z"),
        "profile": "k16-native-passwordless-result-paging-handoff-v8",
