...
if (segments[0][3] != 0x101000 or segments[0][3] + segments[0][6] > 0x10C000
        or segments[1][3] != 0x10C000):
...
if (not any(encoding in data for encoding in cr3_read_encodings)
        ...
        or b"\x4c\x89\x14\x25\x00\xc1\x10\x00" not in data
...
payload = {
    ...
    "protection": {"nxe_readback": True, "cr0_wp_readback": True,
                   ...
                   "kernel_text": "0x101000-0x10bfff-rx",
                   "kernel_state": "0x10c000-rw-nx",
...
print("AIUEOS_KOTOBA_NATIVE_KERNEL_OK no-c no-crt no-linker imports=0 fuel=32768 allocator-pages=8 ownership-bitmap page-table-root identity-1g guard-unmapped text-rx state-rw-nx nxe cr0-wp cr3-activated invlpg idt14-sidt-readback pf-cr2-error-code recovery-frame dedicated-handler-stack reuse double-free-rejected descriptors<=410 zero-before-publish")
