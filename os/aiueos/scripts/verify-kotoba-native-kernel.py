...
if (segments[0][3] != 0x101000 or segments[0][3] + segments[0][6] > 0x110000
        or segments[1][3] != 0x110000):
    raise SystemExit("error: Kotoba-native RX/RW page boundary rejected")
context_offset = segments[1][2]
if segments[1][5] < 16 or struct.unpack_from("<Q", data, context_offset + 8)[0] != 32768:
    raise SystemExit("error: Kotoba-native kernel context does not carry sealed fuel 32768")
...
recovery_handler = b"\x48\x89\x04\x25\x10\xc1\x10\x00" in data
payload = {
    ...
    "fuel": {"initial": 32768, "replenishable": False},
    "allocator": {"page_bytes": 4096, "published_pages": 11,
                  "descriptor_limit": 410,
                  "zero_before_publish": True,
                  "ownership_state": "boot-lifetime-eleven-slot-bitmap",
    ...
    "protection": {"nxe_readback": True, "cr0_wp_readback": True,
                   ...
                   "kernel_text": "0x101000-0x10ffff-rx",
                   "kernel_state": "0x110000-rw-nx",
...
print("AIUEOS_KOTOBA_NATIVE_KERNEL_OK no-c no-crt no-linker imports=0 fuel=32768 allocator-pages=11 ownership-bitmap page-table-root identity-1g guard-unmapped text-rx state-rw-nx nxe cr0-wp cr3-activated invlpg idt14-sidt-readback pf-cr2-error-code recovery-frame dedicated-handler-stack reuse double-free-rejected descriptors<=410 zero-before-publish")
