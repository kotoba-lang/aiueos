...
out=${AIUEOS_OUT:-"$repo/build/aiueos-physical-loader-hang"}
...
AIUEOS_QUALIFICATION_FORCE_LOADER_HANG_CODE=290 \
AIUEOS_QUALIFICATION_LOADER_WATCHDOG_SECONDS=3 \
SOURCE_DATE_EPOCH=0 \
  "$aiueos/scripts/build-physical-qualification-usb.sh" >/dev/null

pristine="$out/aiueos-k16-physical-qualification.img"
work=$(mktemp -d "${TMPDIR:-/tmp}/aiueos-k16-loader-hang.XXXXXX")
...
set -e
[ "$qemu_rc" -eq 124 ] || {
  echo "error: hang collector did not remain at its terminal screen (rc=$qemu_rc)" >&2
  exit 1
}
grep -F "AIUEOS_LOADER_WATCHDOG_ARMED" "$work/debug.log" >/dev/null || {
  echo "error: loader watchdog was not armed" >&2; exit 1;
}
grep -F "AIUEOS_LOADER_PROGRESS forced-hang code=290" "$work/debug.log" >/dev/null || {
  echo "error: forced loader progress marker is absent" >&2; exit 1;
}
grep -F "RESULT SAVED. REMOVE USB AND CONNECT IT TO THE MAC." \
  "$work/serial.log" >/dev/null || {
  echo "error: BootNext probe did not collect the watchdog progress record" >&2; exit 1;
}
...
set -e
[ "$decision_rc" -eq 5 ] || {
  echo "error: checker did not classify loader hang progress (rc=$decision_rc)" >&2
  printf '%s\n' "$decision" >&2
  exit 1
}
printf '%s\n' "$decision" | grep -F \
  "AIUEOS_K16_PHYSICAL_RESULT_INCOMPLETE code=290 reason=loader-hang-progress internal-ssd-writes=none" \
  >/dev/null || { echo "error: checker lost loader hang progress code" >&2; exit 1; }

printf '%s\n' \
  "AIUEOS_K16_LOADER_HANG_RESULT_OK code=290 transport=uefi-variable-watchdog usb-scope=result-partition-only internal-ssd-writes=none"
