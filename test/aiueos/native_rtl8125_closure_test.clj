...
(deftest rtl8125-is-in-the-closed-native-module-graph
  (let [rtl (slurp rtl-source)
    ...
    (testing "the provider is Kotoba source, not a wrapper around the C path"
      ...
      (is (str/includes? rtl "(defn tcp-syn-ack-header-status"))
      (is (str/includes? rtl "inspection (if valid-command"))
      (is (str/includes? rtl "arrived (send-native-nic-diagnostic mmio dma-base 68)"))
      (is (str/includes? rtl "(tcp-syn-ack-header-status\n                              rx-frame"))
      (is (str/includes? rtl "classified (send-native-nic-diagnostic"))
