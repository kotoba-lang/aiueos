...
| 2 — CPU and memory | **Bounded two-page C-free allocator slice working (2026-08-11):** the compiler-owned handoff keeps the final UEFI map in a bounded inline 16 KiB region; Kotoba derives it, walks variable-size descriptors under a 410-descriptor ceiling, and treats the first zeroed page as immutable ownership state while selecting a distinct second page. Both 4,096-byte authorities are derived from the named identity window and zeroed before `MP`. Real x86-64 KVM/OVMF gates prove positive allocation (`MP`, status 33), full-map exhaustion after rejecting every Conventional descriptor (`M`, status 39), and overlap rejection before deriving the second authority (`M`, status 41). The compiler chain seals finite fuel 4096 and the verifier reads that value from the emitted ELF context; metadata alone cannot pass. Privileged intrinsics exist for CR2/CR3, `invlpg`, interrupt control, halt/pause, and port I/O. | persistent ownership bitmap/free list and reclamation; allocate a four-level page-table root; dynamic paging; W^X/guard pages; full exception stubs and double-fault IST; ACPI validation; Local APIC/x2APIC and IOAPIC; timer/IRQ dispatch; AP trampoline, SMP startup, per-CPU GDT/TSS/stacks/state |
