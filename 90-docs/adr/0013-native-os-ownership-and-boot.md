...
| 3 | scheduler, VM, syscall, capability handles | In progress: preemptive CR3-isolated kernel tasks and two APIC-timer-preempted CPL3 processes with distinct roots/private pages/kernel interrupt stacks/domains; switch-time CR3/TSS.RSP0/owner updates; syscall/copy-in and cross-process rejection; W^X; page-backed capability table with generation reuse/retirement and atomic attenuated transfer/claim; scheduler exit/reap, owner-wide generation revocation, zeroed private-page remap and kernel-stack teardown; service restart/IPC/registry. Dynamic process allocation beyond the bounded pool and revocation propagation to downstream derived handles remain |
