...
| 2 — CPU and memory | **First C-free allocator slice working (2026-08-11):** the compiler-owned handoff keeps the final UEFI map in a bounded inline 16 KiB region; Kotoba derives that region, walks variable-size descriptors, selects an aligned Conventional Memory page in the identity-mapped window, checks the page as a subregion of that named window, zeros all 4,096 bytes, and proves three reads plus `MP` in QEMU. Privileged intrinsics exist for CR2/CR3, `invlpg`, interrupt control, halt/pause, and port I/O. Reference implementations exist for the remaining allocator, GDT/IDT, page faults, ACPI, APIC/IOAPIC, timers, and IRQs | allocator ownership state, exhaustion/overlap negative gates, multiple allocation and reclamation; dynamic four-level page tables; W^X/guard pages; full exception stubs and double-fault IST; ACPI validation; Local APIC/x2APIC and IOAPIC; timer/IRQ dispatch; AP trampoline, SMP startup, per-CPU GDT/TSS/stacks/state |
...
The immediate next gate starts where the landed single-page slice stops: retain
allocator ownership state, prove exhaustion and overlap rejection, allocate
the pages for a new four-level page-table root, activate it, and demonstrate
mapping, W^X rejection, unmapping/`invlpg`, and deterministic failure paths in
QEMU without adding a foreign object.
