...
| 2 — CPU and memory | **Bounded three-page C-free ownership slice working (2026-08-11):** the compiler-owned handoff keeps the final UEFI map in a bounded inline 16 KiB region; Kotoba derives it and walks variable-size descriptors under a 410-descriptor ceiling. It admits a contiguous three-page Conventional extent and derives three distinct 4,096-byte authorities from the named identity window. All are zeroed before use. Page one stores a boot-lifetime three-slot bitmap, page two is the allocated zeroed four-level page-table root, and page three proves release and reuse. The positive path rejects duplicate claim and double-free before `MPR`; real x86-64 KVM/OVMF gives `MPR`/33, while a reversed ownership predicate gives only `M`/45. Full-map exhaustion (`M`/39) and overlap rejection (`M`/41) remain independent gates. The compiler chain seals finite fuel 4096 and the verifier reads it from the emitted ELF context. Privileged intrinsics exist for CR2/CR3, `invlpg`, interrupt control, halt/pause, and port I/O. | general map-indexed ownership bitmap/free list and zones; populate and activate the four-level tables; dynamic paging; W^X/guard pages; full exception stubs and double-fault IST; ACPI validation; Local APIC/x2APIC and IOAPIC; timer/IRQ dispatch; AP trampoline, SMP startup, per-CPU GDT/TSS/stacks/state |
...
The immediate next gate starts where the landed two-page sequence stops:
replace its immutable one-sequence ownership value with a persistent bounded
bitmap/free-list, allocate the pages for a new four-level page-table root,
activate it, and demonstrate mapping, W^X rejection, unmapping/`invlpg`, and
deterministic failure paths in QEMU without adding a foreign object.
...
map and admits only a contiguous three-page Conventional Memory extent above
the kernel image and below the current 1 GiB bootstrap identity limit. All
three page authorities are distinct and zeroed. Page one stores a bounded
three-slot ownership bitmap across allocator operations, page two is reserved
as a zeroed four-level page-table root, and page three is released and reused.
...
above the bootstrap map, page-table population, and replacement-CR3 activation
remain.
